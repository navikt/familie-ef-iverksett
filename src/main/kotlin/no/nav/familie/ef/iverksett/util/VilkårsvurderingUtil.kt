...
object VilkårsvurderingUtil {
    fun hentHarSagtOppEllerRedusertFraVurderinger(vilkårsvurderinger: List<Vilkårsvurdering>): Boolean? {
        val vilkårsvurdering =
            vilkårsvurderinger.find { it.vilkårType == VilkårType.SAGT_OPP_ELLER_REDUSERT }
    ...
    fun hentAktivitetsvilkårBarnetilsyn(vilkårsvurderinger: List<Vilkårsvurdering>): AktivitetsvilkårBarnetilsyn? {
        val vilkårsvurdering =
            vilkårsvurderinger.find { it.vilkårType == VilkårType.AKTIVITET_ARBEID }
    ...
    private fun harSagtOppEllerRedusertStilling(svarId: SvarId?) =
        when (svarId) {
            SvarId.JA -> true
            SvarId.NEI -> false
            else -> null
