...
class VedtakstatistikkService(private val vedtakstatistikkKafkaProducer: VedtakstatistikkKafkaProducer, private val featureToggleService: FeatureToggleService) {
    ...
    fun sendTilKafka(iverksettData: IverksettData, forrigeIverksett: IverksettData?) {
        ...
        if (iverksettData is IverksettOvergangsstønad) {
            val vedtakstatistikk = VedtakstatistikkMapper.mapTilVedtakOvergangsstønadDVH(iverksettData, forrigeIverksett?.behandling?.eksternId)
            vedtakstatistikkKafkaProducer.sendVedtak(vedtakstatistikk)
        } else if (iverksettData is IverksettBarnetilsyn) {
            val vedtakstatistikk = VedtakstatistikkMapper.mapTilVedtakBarnetilsynDVH(iverksettData, forrigeIverksett?.behandling?.eksternId)
            vedtakstatistikkKafkaProducer.sendVedtak(vedtakstatistikk)
            ...
            if (featureToggleService.isEnabled("familie.ef.iverksett.vedtaksstatistikk-skolepenger")) {
                val vedtakstatistikk = VedtakstatistikkMapper.mapTilVedtakSkolepengeDVH(iverksettData, forrigeIverksett?.behandling?.eksternId)
                vedtakstatistikkKafkaProducer.sendVedtak(vedtakstatistikk)
            } else {
