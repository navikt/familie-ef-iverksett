...
internal class DistribuerVedtaksbrevTaskTest {
    ...
    internal fun `skal distribuere brev`() {
        ...
        every { tilstandRepository.hentJournalpostResultat(behandlingId) } returns mapOf("123" to JournalpostResultat(
                journalpostId))
        ...
        every { tilstandRepository.hentdistribuerVedtaksbrevResultat(behandlingId) } returns null andThen mapOf(journalpostId to DistribuerVedtaksbrevResultat(
                bestillingId))
        every {
    ...
    fun `skal kun distribuere brev til mottakere som ikke allerede er distribuert til`() {
        ...
        val journalpostResultater =
                mapOf(identMottakerA to JournalpostResultat(distribuertJournalpost), "456" to JournalpostResultat(
                        ikkeDistribuertJournalpost))
        val distribuerteJournalposter =
                mapOf(journalpostResultater[identMottakerA]!!.journalpostId to DistribuerVedtaksbrevResultat(
